...
1.  **Read Before Write (`view_file`):** NEVER rely on memory. Before any edit, read the file again to ensure you have the exact lines and context at the moment.
2.  **Small Changes (`replace_file_content`):**
    - Use a **small, unique `TargetContent`** (2-4 lines) to serve as an anchor.
    - DO NOT attempt to replace huge blocks (over 10-15 lines) at once – the risk of mismatch is huge.
3.  **Large Changes or Refactoring (`write_to_file`):**
    - If the file is small (under 300 lines) or logic changes drastically – **rewrite the entire file**.
    - This is safer than "patching" because it guarantees code integrity.
4.  **Avoid `multi_replace_file_content` for Logic:** This tool often confuses context. Use it ONLY for simple text replacements (e.g., variable renaming).
5.  **Verify Immediately:** After every edit, MANDATORY check the result (`view_file`) to catch broken syntax immediately.
6.  **NO PowerShell for File Writing:** NEVER use PowerShell commands (like `Set-Content` or `Out-File`) to write or edit files. They often insert invisible Byte Order Marks (BOM) that break JSON parsers and Node.js. Use `write_to_file` tool or Node.js scripts (`fs.writeFileSync`) instead.
7.  **Verify JSON Immediately:** After editing any JSON file, immediately verify its validity to catch syntax errors early.
...
Когато потребителя ти зададе въпрос "Какво мислиш?", това означава да НЕ ПРЕДПРИЕМАШ действия като редакция или промяна на код, а само да изразиш мнение и твоето виждане по зададения въпрос!!!

## Documentation First / Sync Protocol

This protocol applies to ALL documentation in `Docs/Instructions/` and other critical references.

**Rule:** When you implement an architectural change, new feature, or configuration schema update (e.g., adding `valuePath` to drivers):

1.  **Implement & Test:** First, realize the change in the codebase and verify it works.
