...
## 3а. ВАЖНО: Откриване на Устройства (Discovery) 📡

За да може системата автоматично да намира контролери в локалната мрежа (чрез UDP Broadcast), Docker контейнерите трябва да имат директен достъп до мрежовия интерфейс на Raspberry Pi-то.

По подразбиране `docker-compose.rpi.yml` използва стандартна Bridge мрежа. Това изолира контейнерите и Broadcast пакетите (255.255.255.255) **НЕ излизат** извън Raspberry-то.

### Решение: Host Networking Mode (Препоръчително)
Ако Network Scanner не намира вашите ESP32/Arduino контролери, трябва да разрешите `host` мрежов режим.

1. Отворете `docker-compose.rpi.yml`:
   ```bash
   nano docker-compose.rpi.yml
   ```
2. Намерете секцията за `backend` и добавете/разкоментирайте:
   ```yaml
   network_mode: "host"
   ```
   *(Забележка: В режим `host`, порт mapping-ите `ports: ...` се игнорират, тъй като контейнерът използва портовете на хоста директно).*

3. Рестартирайте:
   ```bash
   docker compose -f docker-compose.rpi.yml up -d --force-recreate
   ```

След това Refresh-нете страницата. В Network Scanner ще видите, че Broadcast IP е автоматично разпознато като (напр.) `192.168.1.255` вместо `172.18.x.x`.
