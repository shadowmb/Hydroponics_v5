                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex-1 overflow-y-auto bg-background p-6 min-h-0">
                            ...
                            <TabsContent value="settings" className="m-0 h-full overflow-y-auto p-4 space-y-6">
                                {/* --- NEW: General Settings (Roles) --- */}
                                {(() => {
                                    // Resolve Template for Roles
                                    const template = typeof device?.config?.driverId === 'object'
                                        ? device.config.driverId
                                        : null; // If string, we can't show roles easily without fetching. But extractTemplateInfo tries to get it.

                                    // Use 'hardwareLimits' logic which already accesses template? 
                                    // Actually, let's use the template object if we have it.
                                    // The 'device' prop usually comes from 'useDevice' hook which populates driverId.

                                    if (template?.roles && Object.keys(template.roles).length > 0) {
                                        const currentRole = device.config?.activeRole || 'default';

                                        return (
                                            <div className="space-y-4 max-w-2xl mx-auto border rounded-lg p-4 bg-card">
                                                <div className="flex items-center gap-2 mb-4">
                                                    <div className="h-8 w-8 rounded bg-blue-500/10 flex items-center justify-center text-blue-500">
                                                        <Square className="h-4 w-4" />
                                                    </div>
                                                    <div>
                                                        <h3 className="font-medium">Device Role</h3>
                                                        <p className="text-sm text-muted-foreground">Select how this sensor is used</p>
                                                    </div>
                                                </div>

                                                <div className="grid grid-cols-1 gap-4">
                                                    <div className="space-y-2">
                                                        <label className="text-xs font-medium">Active Role</label>
                                                        <Select
                                                            value={currentRole}
                                                            onValueChange={async (val) => {
                                                                try {
                                                                    const selectedRole = template.roles[val];
                                                                    const body: any = { "config.activeRole": val };
                                                                    ...
                                                                    const msg = selectedRole?.defaultStrategy
                                                                        ? `Role set to ${selectedRole.label} (Strategy: ${selectedRole.defaultStrategy})`
                                                                        : `Role set to ${selectedRole.label}`;
                                                        ...
                                                        >
                                                            ...
                                                            <SelectContent>
                                                                {/* Optional: Add 'Unassigned' or 'Default' if needed? */}
                                                                {Object.entries(template.roles).map(([key, role]: any) => (
