<template>
  <q-card style="min-width: 500px; max-width: 600px">
    <q-card-section class="row items-center">
      <div class="text-h6">
        {{ device ? 'Редактиране на устройство' : 'Добавяне на устройство' }}
      </div>
      <q-space />
    ...
    <q-card-section class="q-pt-md">
      <q-form @submit="handleSave" class="q-gutter-md">
        ...
        <div class="row q-gutter-sm">
          ...
          <div style="width: 150px;">
            <q-select
              ...
              label="Мерна единица *"
              :rules="[(val) => (val !== null && val !== '' && val !== undefined) || 'Мерната единица е задължителна']"
              outlined
              emit-value
              map-options
            >
        ...
        >
          <!-- Custom template за да покажем tooltip -->
          <template v-slot:option="{ itemProps, opt }">
            <q-item v-bind="itemProps" :disable="opt.disable">
              ...
              <q-tooltip v-if="opt.disable">
                Този контролер няма подходяща команда
              </q-tooltip>
            </q-item>
          </template>
        </q-select>
        ...
        <q-select
          ...
          label="Метод на свързване *"
          :rules="[(val) => !!val || 'Методът на свързване е задължителен']"
          outlined
          emit-value
          map-options
        ...
        <q-select
          ...
          label="Избери реле *"
          :rules="[(val) => !!val || 'Релето е задължително']"
          outlined
          emit-value
          map-options
        ...
        <div v-if="selectedTemplate && formData.connectionMethod === 'direct'" class="q-mt-md">
          ...
          <div class="text-subtitle2 q-mt-md q-mb-sm">
            Портове за {{ selectedTemplate.displayName }}
          </div>
          ...
          <div v-for="portReq in selectedTemplate.portRequirements" :key="portReq.role" class="q-mb-md">
            <q-select
              ...
              :label="`${portReq.role} порт (${portReq.type}) ${portReq.required ? '*' : ''}`"
              :rules="[]"
              outlined
              emit-value
              map-options
        ...
        <div v-if="formData.category === 'sensor'" class="q-mt-md">
          <q-expansion-item
            ...
            label="Конфигурация за валидация на сензора"
            ...
            <div class="q-pa-md bg-grey-1 rounded-borders">
              <div class="column q-gutter-md">
                ...
                <q-toggle
                  ...
                  label="Включи автоматична валидация на сензора"
                  color="positive"
                />

                <div v-if="formData.healthCheckEnabled" class="column q-gutter-md">
                  <!-- Expected Range -->
                  <div class="text-subtitle2 q-mb-sm">Очакван диапазон на стойностите</div>
                  <div class="row q-gutter-md">
                    <q-input
                      ...
                      :rules="[(val) => val !== null && val !== undefined || 'Задължително поле']"
                    />
                    <q-input
                  ...
                  <!-- Physical Range -->
                  <div class="text-subtitle2 q-mb-sm">Физически диапазон (хардуерни лимити)</div>
                  <div class="row q-gutter-md">
                    <q-input
                  ...
                  <!-- Advanced Settings -->
                  <div class="text-subtitle2 q-mb-sm">Допълнителни настройки</div>
                  <div class="column q-gutter-md">
                    <q-input
                      v-model.number="formData.validationConfig.maxChangeRate"
                      label="Макс. промяна в минута"
                      type="number"
                      outlined
                      dense
                      style="max-width: 180px"
                      hint="Максимална допустима промяна на стойността за минута"
                    />
                    <q-input
                      v-model.number="formData.validationConfig.historyWindow"
                      label="Часове за анализ"
                      type="number"
                      outlined
                      dense
                      style="max-width: 180px"
                      hint="Колко часа назад да анализира данните"
                    />
                    <q-input
                      v-model.number="formData.validationConfig.stuckValueThreshold"
                      label="Лимит за блокирана стойност (мин)"
                      type="number"
                      outlined
                      dense
                      style="max-width: 180px"
                      hint="След колко минути да се счита стойността за блокирана"
                    />
                  </div>
                </div>
              </div>
        ...
        <div class="column q-gutter-sm">
          ...
          <div v-if="!hasConfiguredPorts" class="text-caption text-orange q-ml-md">
            ...
            Устройството няма конфигурирани портове и ще бъде деактивирано автоматично
          </div>
        </div>
      </q-form>
    </q-card-section>
...
const physicalTypeOptions = computed(() => {
  // Get unique category values from deviceTemplates uiConfig
  const uniqueCategories = [...new Set(deviceTemplates.value.map(template => template.uiConfig?.category || template.physicalType))]

  // Add "Всички" option at the beginning
  const options = [{
    label: 'Всички типове',
    value: 'all'
  }]

  // Add actual categories (directly use category name, no mapping needed)
  options.push(...uniqueCategories.map(category => ({
    label: category,
    value: category
  })))
...
const deviceTypeOptions = computed(() => {
  // If no category selected or "all" selected, show all templates
  ...
  // Filter templates by selected category (using uiConfig.category)
  return deviceTemplates.value
    .filter(template => {
      const templateCategory = template.uiConfig?.category || template.physicalType
      return templateCategory === formData.value.physicalType
    })
    .map(template => ({
      label: template.displayName,
      value: template.type,
...
const controllerOptions = computed(() => {
  ...
  return props.controllers.map(controller => {
    // Проверка дали контролерът има нужната команда
    const hasCommand = requiredCommand
      ? (controller.capabilities?.includes(requiredCommand) ?? false)
      : true  // Ако няма requiredCommand, всички са валидни

    return {
      label: controller.name,
      value: controller._id,
      controller: controller,
      disable: !hasCommand  // disabled ако няма командата
    }
  })
})

const priorityOptions = computed(() => [
  { label: 'Критично', value: 'critical', color: 'red' },
  { label: 'Високо', value: 'high', color: 'orange' },
  { label: 'Нормално', value: 'normal', color: 'blue' },
  { label: 'Слабо', value: 'low', color: 'grey' }
])

const connectionMethodOptions = computed(() => [
  { label: 'Директно към контролер', value: 'direct' },
  { label: 'Чрез реле', value: 'relay' }
])

const iconOptions = computed(() => [
  { label: 'Термометър', value: 'thermostat' },
  { label: 'Наука', value: 'science' },
  { label: 'Електричество', value: 'electrical_services' },
  { label: 'Капка вода', value: 'water_drop' },
  { label: 'Разстояние', value: 'straighten' },
  { label: 'Енергия', value: 'power' },
  { label: 'Устройство', value: 'device_hub' },
  { label: 'Датчик', value: 'sensors' },
  { label: 'Включен', value: 'power_on' },
  { label: 'Изключен', value: 'power_off' },
  { label: 'Батерия', value: 'battery_full' },
  { label: 'WiFi', value: 'wifi' },
  { label: 'Bluetooth', value: 'bluetooth' },
  { label: 'USB', value: 'usb' },
  { label: 'Настройки', value: 'settings' },
  { label: 'Инструменти', value: 'build' },
  { label: 'Контрол', value: 'tune' },
  { label: 'Мониторинг', value: 'monitor' },
  { label: 'Цифрово', value: 'memory' },
  { label: 'Аналогово', value: 'show_chart' },
  { label: 'Агроном', value: 'agriculture' },
  { label: 'Растение', value: 'local_florist' },
  { label: 'Слънце', value: 'wb_sunny' },
  { label: 'Светлина', value: 'lightbulb' },
  { label: 'Вентилатор', value: 'air' },
  { label: 'Помпа', value: 'water_pump' },
  { label: 'Филтър', value: 'filter_alt' },
  { label: 'Клапан', value: 'valve' },
  { label: 'Хранене', value: 'restaurant' },
  { label: 'Резервоар', value: 'propane_tank' }
])

// Dynamic unit options from all device templates (flatten all defaultUnits arrays)
const baseUnitOptions = computed(() => {
  const allUnits = deviceTemplates.value
    .filter(template => template.defaultUnits && Array.isArray(template.defaultUnits))
    .flatMap(template => template.defaultUnits)
...
const relayPortOptions = computed(() => {
  ...
  const relayPorts = selectedRelay.value.ports
    .map((port: RelayPort) => {
      ...
      return {
        label: `Порт ${port.portNumber} (${port.controlPin || 'Няма пин'})`,
        value: port.portNumber,
        portKey: `Порт ${port.portNumber}`,
        portType: 'digital',
        currentState: hasControlPin ? port.currentState : 'N/A',
        stateColor: stateColor,
        description: port.controlPin || 'Няма конфигуриран пин',
        disable: isDisabled,
        port: port
      }
    })
...
async function handleSave() {
  ...
  try {
    ...
    if (!formData.value.unit || formData.value.unit === '') {
      $q.notify({
        type: 'negative',
        message: 'Моля изберете мерна единица'
      })
      saving.value = false
      return
    }
    ...
    if (selectedTemplate.value?.executionConfig?.commandType) {
      ...
      if (selectedController && !selectedController.capabilities?.includes(requiredCommand)) {
        $q.notify({
          type: 'negative',
          message: `Избраният контролер "${selectedController.name}" няма нужната команда "${requiredCommand}" за този тип устройство. Моля изберете друг контролер.`
        })
        saving.value = false
        return
      }
    ...
    if (formData.value.physicalType === 'all') {
      $q.notify({
        type: 'negative',
        message: 'Моля изберете конкретен физически тип, а не "Всички типове"'
      })
      return
    }
    ...
    if (formData.value.connectionMethod === 'relay') {
      if (!formData.value.relayId) {
        $q.notify({
          type: 'negative',
          message: 'Моля изберете реле за релейно свързване'
        })
        return
      }
      ...
      if (formData.value.relayPort === null) {
        ...
        const confirmed = await new Promise((resolve) => {
          $q.dialog({
            ...
            message: 'Не е избран порт на релето. Искате ли да продължите?',
            cancel: true,
            persistent: true
          }).onOk(() => resolve(true))
      ...
      if (selectedTemplate.value) {
        ...
        if (missingPorts.length > 0) {
          ...
          const confirmed = await new Promise((resolve) => {
            $q.dialog({
              title: 'Предупреждение',
              message: `Не са избрани задължителни портове: ${portsList}. Искате ли да продължите?`,
              cancel: true,
              persistent: true
            }).onOk(() => resolve(true))
             .onCancel(() => resolve(false))
...
async function loadDeviceTemplates() {
  ...
  try {
    ...
    $q.notify({
      type: 'negative',
      message: 'Грешка при зареждане на типовете устройства'
    })
  } finally {
...
function onDeviceTypeChange(newType: string) {
  ...
  // IMPORTANT: Auto-set physicalType from template (for backend converter routing)
  // Note: formData.physicalType is used for UI filtering, but must be overwritten with template value
  if (template) {
  ...
  // Auto-set default unit from template (first unit in array)
  if (template && template.defaultUnits && template.defaultUnits.length > 0) {
    formData.value.unit = template.defaultUnits[0]
    console.log(`[DeviceForm] Auto-setting unit to: ${template.defaultUnits[0]} (from ${template.defaultUnits.join(', ')})`)
  }
...
function getIconLabel(iconValue: string): string {
  if (!iconValue) return 'Устройство'
...
// Watch for relay port changes - handle "Празен" selection
...
watch(() => formData.value.healthCheckEnabled, (newEnabled, oldEnabled) => {
  console.log(`[DeviceForm] 🔍 Health check toggle changed: ${oldEnabled} -> ${newEnabled}`)
  console.log('[DeviceForm] 🔍 Current healthCheckEnabled:', formData.value.healthCheckEnabled)
})
